...
const views = [
    { id: 'overview', label: 'Overview', description: 'Ringkasan sekolah, penerima manfaat, konfirmasi, feedback, dan insiden.' },
...
function renderActionBar() {
    return `<div class="inventory-action-bar"><button class="erp-btn primary" data-action="refresh-school" type="button">Refresh Data</button><button class="erp-btn" data-jump-view="schools" type="button">Tambah Sekolah</button><button class="erp-btn" data-jump-view="receipts" type="button">Konfirmasi Terima</button><button class="erp-btn danger" data-jump-view="incidents" type="button">Laporkan Insiden</button></div>`;
}
...
function renderSchoolOverview() {
    const summary = getSchoolSummaryFromState(pageState.state);
    const pendingReceipts = pageState.state.dispatchOrders.filter((row) => !['delivered', 'selesai', 'terkirim'].includes(String(row.status || '').toLowerCase())).length;
    const cards = [
        ['Sekolah', summary.schools, 'Sekolah terdaftar'],
        ['Penerima Manfaat', summary.totalStudents.toLocaleString('id-ID'), 'Target siswa'],
        ['School Receipt', pendingReceipts, 'Menunggu konfirmasi'],
        ['Feedback', pageState.state.feedback.length, 'Masukan sekolah'],
        ['Insiden', summary.openIncidents, 'Perlu tindak lanjut']
    ];
    return `
        <section class="kt-card">
            <div class="kt-card-header flex-wrap gap-3">
                <div>
                    <h3 class="kt-card-title">Overview Sekolah</h3>
                    <p class="kt-card-description">Monitoring penerima manfaat, konfirmasi penerimaan, feedback, dan insiden sekolah.</p>
                </div>
                <a class="kt-btn kt-btn-sm kt-btn-primary" href="#school/receipts">Konfirmasi Terima</a>
            </div>
...
function renderActiveView() {
    if (pageState.activeView === 'overview') return renderSchoolOverview();
...
function renderSchoolContent() {
    ...
    if (pageState.loading) {
        root.innerHTML = LoadingCard({ title: 'Memuat Sekolah dari Data', text: 'Mengambil sekolah, penerima, konfirmasi, feedback, dan insiden...' });
        return;
    }
    if (pageState.error) {
...
export function initSchoolPage(initialView = 'overview') {
