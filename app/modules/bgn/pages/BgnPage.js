...
const pageState = { activeView: 'overview', loading: true, error: '', state: { bgnReports: [], complianceChecks: [], regionalKpis: [], schools: [], dispatchOrders: [], incidents: [], wasteRecords: [] } };
const views = [
    { id: 'overview', label: 'Overview', description: 'Ringkasan monitoring, compliance, KPI, laporan, dan risiko.' },
    { id: 'checklist-penerimaan-bahan', label: 'Checklist Penerimaan Bahan', description: 'Compliance penerimaan dan bahan pangan.' },
    { id: 'monitor-suhu', label: 'Monitor Suhu', description: 'Pantauan suhu produksi, penerimaan, dan penyimpanan.' },
    { id: 'bank-sampel', label: 'Bank Sampel', description: 'Kontrol sampel makanan untuk audit.' },
    { id: 'sanitasi', label: 'Sanitasi', description: 'Checklist kebersihan dan status koreksi.' },
    { id: 'audit-trail', label: 'Audit Trail', description: 'Riwayat compliance dan laporan.' },
...
const viewAliases = {
    'checklist-penerimaan-bahan': 'compliance',
    'monitor-suhu': 'monitoring',
    'bank-sampel': 'compliance',
    'sanitasi': 'compliance',
    'audit-trail': 'reports'
};

function normalizeBgnView(view) {
...
function renderActiveTable() {
    const activeView = normalizeBgnView(pageState.activeView);
...
function renderActiveView() {
    ...
    if (activeView === 'overview') {
        return `<div class="erp-card"><h3>Overview BGN Dashboard</h3><div class="erp-grid"><article class="erp-card"><div class="erp-stat-label">Checklist Penerimaan</div><div class="erp-stat-value">${pageState.state.complianceChecks.length}</div><div class="erp-stat-note">Compliance bahan</div></article><article class="erp-card"><div class="erp-stat-label">Monitor Suhu</div><div class="erp-stat-value">${pageState.state.dispatchOrders.length}</div><div class="erp-stat-note">Batch dan distribusi dipantau</div></article><article class="erp-card"><div class="erp-stat-label">Bank Sampel</div><div class="erp-stat-value">${pageState.state.bgnReports.length}</div><div class="erp-stat-note">Referensi laporan</div></article><article class="erp-card"><div class="erp-stat-label">Audit Trail</div><div class="erp-stat-value">${pageState.state.incidents.length}</div><div class="erp-stat-note">Insiden dan tindak lanjut</div></article></div>${renderActiveTable()}</div>`;
    }
    const forms = { compliance: renderComplianceForm, regional: renderRegionalForm, reports: renderReportForm };
    const form = forms[activeView] ? forms[activeView]() : '';
...
function renderBgnContent() {
    ...
    if (!root) return;
    if (pageState.loading) { root.innerHTML = LoadingCard({ title: 'Memuat BGN dari Data', text: 'Mengambil laporan, compliance, KPI regional, dan agregat operasional...' }); return; }
...
export function initBgnPage(initialView = 'overview') {
