...
const pageState = { activeView: 'daily', loading: true, error: '', state: { dailyReports: [], monthlyReports: [], exports: [], dispatchOrders: [], productionBatches: [], wasteRecords: [], incidents: [], nutritionChecks: [] } };
const views = [
...
function renderActionBar() {
    return `<div class="inventory-action-bar"><button class="erp-btn primary" data-action="refresh-reports" type="button">Refresh GAS</button><button class="erp-btn" data-jump-view="daily" type="button">Laporan Harian</button><button class="erp-btn" data-jump-view="monthly" type="button">Laporan Bulanan</button><button class="erp-btn" data-action="download-report-summary" type="button">Unduh Ringkasan CSV</button></div>`;
}
...
function renderActiveTable() {
    if (pageState.activeView === 'monthly') return table(['Periode', 'Status', 'File', 'Dibuat'], pageState.state.monthlyReports.slice(0, 12).map((row) => `<tr><td>${esc(row.period)}</td><td>${esc(row.status)}</td><td>${row.file_url ? `<a href="${esc(row.file_url)}" target="_blank">File</a>` : '-'}</td><td>${esc(row.created_at)}</td></tr>`).join(''), 'Belum ada laporan bulanan.');
    if (pageState.activeView === 'exports') return table(['Tipe', 'Format', 'Status', 'Peminta', 'Dibuat'], pageState.state.exports.slice(0, 12).map((row) => `<tr><td>${esc(row.report_type)}</td><td>${esc(row.format)}</td><td>${esc(row.status)}</td><td>${esc(row.requested_by)}</td><td>${esc(row.created_at)}</td></tr>`).join(''), 'Belum ada export.');
    if (pageState.activeView === 'dashboard') return table(['Sumber', 'Indikator', 'Nilai', 'Catatan'], [
...
function renderActiveView() {
    if (pageState.activeView === 'monthly') return `${renderMonthlyForm()}<div class="erp-card module-card-gap"><h3>Laporan Bulanan</h3>${renderActiveTable()}</div>`;
    if (pageState.activeView === 'exports') return `${renderExportForm()}<div class="erp-card module-card-gap"><h3>Export</h3>${renderActiveTable()}</div>`;
    if (pageState.activeView === 'dashboard') return `<div class="erp-card"><h3>Dashboard Report</h3>${renderActiveTable()}</div>`;
...
function renderReportsContent() {
    ...
    if (!root) return;
    if (pageState.loading) { root.innerHTML = '<div class="erp-card inventory-loading"><h3>Memuat Reports dari GAS</h3><p class="erp-muted">Mengambil laporan, export, dan ringkasan modul...</p></div>'; return; }
